...
public class PoiService {
    ...
    public ApiResponse<Void> deletePoi(@NonNull Integer routeId,@NonNull Integer poiId)
    {
        ...
        try
        {
            List<Image> images = imageRepo.findByEntityTypeAndEntityId(EntityType.POI,poi.getId()).stream().toList();
            poiRepo.delete(poi);
            images.forEach(this::deleteImagesForPoi);
            return new ApiResponse<>(null, "poi deleted", 200);
        }
        catch (Exception e)
        {
            log.error("exception thrown while deleting poi {} , {}",e.getClass(),e.getLocalizedMessage());
            throw new ResourceDeletionFailedException("poi","id",poiId);
        }
    }
    ...
    @Transactional(readOnly = true)
    public ApiResponse<List<PoiResponse>> getPoiNearby(NearbyRequest nearbyRequest)
    {
        ...
        List<PoiResponse> poiResponses = nearbyPois.stream().map(nearbyPoi ->
        {
            POI poi = poiRepo.findById(nearbyPoi.getId()).orElse(null);
        ...
        if(poiResponses.isEmpty()) throw new NoResourceFoundException("nearby pois");
        return new ApiResponse<>(poiResponses,"nearby poi fetched",200);

    }


    // helper method for image deleting
    public void deleteImagesForPoi(Image image)
    {
        try{
